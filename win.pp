...
class appd_agent::windows(
  ...
  $win_db_agent_binary_file          = $appd_agent::win_db_agent_binary_file,
  $win_db_agent_install_jvm_options  = $appd_agent::win_db_agent_install_jvm_options,
  ...
  $win_db_agent_install_dir          = $appd_agent::win_db_agent_install_dir,
  ...
  # Create Directory
  #$appd_agent_dirs = unique([ $binary_dir, $app_agent_install_dir, $app_dotnet_agent_dir, $db_agent_install_dir, $machine_agent_install_dir ])
  ...
  if versioncmp($_version, $_old_version) > 0 {  # Need to upgrade, so backup
    ...
    appd_agent::windows::service_action { "Stop Service $db_agent_service_name":
  ...
  appd_agent::windows::unzip { $db_agent_binary_file:
...
define appd_agent::windows::service_action(
  $service,
  $status
) {
  ...
  case $status {
                  'stop': { 
				            notify {"STOP: $status service: $service": }
