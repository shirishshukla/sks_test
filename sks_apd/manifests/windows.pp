...
class ahead_appd::windows(
  ...
  $app_dotnet_agent_dir           = "${binary_dir}\\dotNetAgentMSIFiles"
  ...
  # Start Enable DB Agent as Service
  ...
  if $facts['apache_tomcat_installed_status'] == 'Installed' {
    ...
    ## Install Java Agent
    $java_agent_version                  = regsubst($win_java_agent_binary_file.split('-')[-1], '^(.+)\.zip$', '\1')
    ...
    if $facts['win_java_agent_version'] != undef {
      $j_old_version = $facts['win_java_agent_version']
    } else {
    ...
    $msi_file_path                  = "${app_dotnet_agent_dir}\\${win_dot_net_agent_binary_file}"
    $msi_agent_installer_log_file   = "${app_dotnet_agent_dir}\\AgentInstaller.log"
    $ad_config_file                 = "${app_dotnet_agent_dir}\\AD_Config.xml"
  ...
  } # Close If condition of app agent
  ...
  $machine_agent_version            = regsubst($machine_agent_binary_file.split('-')[-1], '^(.+)\.zip$', '\1')

  if $facts['win_machine_agent_version'] != undef {
    $m_old_version = $facts['win_machine_agent_version']
  } else {
  ...
  if versioncmp($m_version, $m_old_version) > 0 {  # Need to upgrade, so backup
    ...
    # UnInstall Machine Agent as service if already installed
    exec { 'Un-install Machine Agent as a Service':
  ...
  ahead_appd::windows::unzip { $machine_agent_binary_file:
  ...
  file { $win_machine_controller_info_file:
    ...
    require => Ahead_appd::Windows::Unzip[$machine_agent_binary_file] #, Exec["Unzip ${machine_agent_binary_file}"],
  }
