...
class jboss::archive_standalone_sre (
  ...
  exec { 'standalone file compare':
    command  => "test -f ${standalone_full_ha_file_back} && rm -rf ${standalone_full_ha_file} ${dep_valid}/datasource* ${standalone_full_ha_prime_source}",
    unless   => "test $(md5sum ${standalone_full_ha_file} | cut -b -32) == $(md5sum ${standalone_full_ha_file_back} | cut -b-32)",
    provider => 'shell',
    notify   => File[$standalone_full_ha_prime_source],
  }
  ...
  file { "${dep_valid}/standalone-full-ha.xml_updates.sh":
...
class jboss::archive_standalone_sre::datasource00(
  ...
  $dep_valid               = "${config_path}/dep_valid"
  $standalone_full_ha_file = "${config_path}/standalone-full-ha.xml"
  $cmd = "sh ${dep_valid}/standalone-full-ha.xml_updates.sh \'${standalone_full_ha_file}\' \'${jndi_name}\' \'${pool_name}\' \'${connection_url}\' \'${driver}\' \'${min_pool_size}\' \'${max_pool_size}\' \'${security_domain}\' \'${username}\' \'${password}\'"

  if $datasource00_enabled {
  
    file { "$dep_valid/${dcname}.status":
      content => "$datasource00_enabled - $cmd",
      require => File["${dep_valid}/standalone-full-ha_prime_source.xml"],
      notify  => Exec["Create $standalone_full_ha_file"]
    }
  
    if $jndi_name and $pool_name and $connection_url and $driver and $security_domain {
	  jboss::archive_standalone_sre::back { $name: }
      exec { $cmd:
        refreshonly => true,
		...
		notify      => Jboss::Archive_standalone_sre::Back[$name]
      }
    } else {
...
## Take backup of standalone file 
define jboss::archive_standalone_sre::back () {
  exec { "Take backup of $standalone_full_ha_file $name":
